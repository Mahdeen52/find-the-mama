...
export async function DELETE(_: Request, { params }: { params: { id: string } }) { try { await requireAuth(); await db.vendor.delete({ where: { id: params.id } }); return ok({ deleted: true }); } catch (error) { return handleApiError(error); } }
